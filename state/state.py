...
categories = df.groupby('state')['state'].count()
red = Color("gold")
...
ax =categories.plot(kind='bar', color=colors,title= 'Quantity of projects in states',fontsize=20)
...
ax2 = categories.plot(kind='box',title= 'Quantity of projects in state' )
ax2.title.set_size(30)
ax2.set_ylabel("Number of projects",fontsize=20)
plt.savefig('state-box.png')