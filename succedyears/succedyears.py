...
df3 = DataFrame({"year":years,"state":df.state})
print(df3)
...
ax =categories.plot(kind='bar', color=colors,title= 'Quantity of successful project through years',fontsize=20)
ax.set_xlabel("Years",fontsize=25)
ax.set_ylabel("Number of projects",fontsize=25)
ax.title.set_size(40)

plt.savefig('successful.png')

plt.clf()


figure(figsize=(10,8),dpi=200)
ax2 = categories.plot(kind='box',title= 'Quantity of successful project through years' )
ax2.title.set_size(30)
ax2.set_ylabel("Number of projects",fontsize=20)
plt.savefig('successful-box.png')
